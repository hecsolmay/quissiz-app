          ...
          {tests.map(test => (
            ...
            >
              <img
                ...
                className='absolute inset-0 z-0 size-full rounded-lg object-fill transition-all duration-200 group-hover:scale-105 group-hover:opacity-75'
              />
              <div className='absolute bottom-0 left-0 z-10 my-auto flex h-14 w-full items-center rounded-b-lg bg-black/70 px-4 py-2 text-sm text-white'>
                <h3 className='truncate text-lg font-medium'>{test.name}</h3>
              </div>
            </div>
          ))}
        </div>
